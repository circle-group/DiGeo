...
template <typename T>
CPH_DEVICE constexpr T epsilon();

template <>
CPH_DEVICE constexpr float epsilon<float>() { return 1e-4f; }

template <>
CPH_DEVICE constexpr double epsilon<double>() { return 1e-7; }
...
struct vec3 {
    ...
    CPH_DEVICE vec3() : values{T(0), T(0), T(0)} {}

    CPH_DEVICE vec3(T x, T y, T z) : values{x, y, z} {}

    CPH_DEVICE void zero() {
        values[0] = T(0);
        values[1] = T(0);
        values[2] = T(0);
    }

    CPH_DEVICE vec3<T> copy() const {
        return vec3<T>(values[0], values[1], values[2]);
    }

    CPH_DEVICE T& operator[](int index) {
        return values[index];
    }

    CPH_DEVICE const T& operator[](int index) const {
        return values[index];
    }

    CPH_DEVICE vec3<T> operator+(const vec3<T>& other) const {
    ...
    CPH_DEVICE vec3<T> operator-() const {
        return vec3<T>(-values[0], -values[1], -values[2]);
    }

    CPH_DEVICE vec3<T> operator*(T scalar) const {
...
template <typename T>
CPH_DEVICE vec3<T> operator*(T scalar, const vec3<T>& v) {
    return vec3<T>(v[0] * scalar, v[1] * scalar, v[2] * scalar);
}

template <typename T>
CPH_DEVICE T length(const vec3<T>& v) {
    return sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]);
}

template <typename T>
CPH_DEVICE vec3<T> normalize(const vec3<T>& v) {
    T l = length(v);
...
template <typename T>
CPH_DEVICE vec3<T> cross(const vec3<T>& a, const vec3<T>& b) {
    return vec3<T>(
        a[1] * b[2] - a[2] * b[1],
        a[2] * b[0] - a[0] * b[2],
        a[0] * b[1] - a[1] * b[0]
...
template <typename T>
CPH_DEVICE T dot(const vec3<T>& a, const vec3<T>& b) {
    return a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
}

template <typename T>
CPH_DEVICE vec3<T> project_vec(const vec3<T>& vec, const vec3<T>& normal) {
    vec3<T> n = normalize(normal);
    return vec - geodesic_utils::dot(vec, n) * n;
}

template <typename T>
CPH_DEVICE vec3<T> triangle_normal(const vec3<T>& p0, const vec3<T>& p1, const vec3<T>& p2) {
    return normalize(geodesic_utils::cross(p1 - p0, p2 - p0));
}

template <typename T>
struct Mesh {
    const torch::PackedTensorAccessor32<T,2,torch::RestrictPtrTraits> positions;
    const torch::PackedTensorAccessor32<int,2,torch::RestrictPtrTraits> triangles;
    const torch::PackedTensorAccessor32<int,2,torch::RestrictPtrTraits> adjacencies;
    ...
    CPH_DEVICE vec3<T> get_vertex(int vertex_id) const {
        return vec3<T>(
    ...
    CPH_DEVICE vec3<T> get_triangle_normal(int triangle_id) const {
        return vec3<T>(
    ...
    CPH_DEVICE vec3<T> get_vertex_normal(int vertex_id) const {
        return vec3<T>(
...
struct MeshPoint {
    ...
    __device__ MeshPoint(int face_id, T u, T v) : face(face_id), u(u), v(v) {}

    CPH_DEVICE vec3<T> interpolate(const Mesh<T>& mesh) const {
    ...
    CPH_DEVICE vec3<T> get_barycentric_coords() const {
        return vec3<T>(
            T(1) - u - v,
            u,
            v
...
template <typename T>
CPH_DEVICE vec3<T> rotate_vector(const vec3<T>& v, const T angle, const vec3<T>& n) {
    return v * cos(angle) + cross(n, v) * sin(angle) + n * dot(n, v) * (T(1.0f) - cos(angle));
}


template <typename T>
CPH_DEVICE vec3<T> tri_bary_coords(
...
template <typename T>
CPH_DEVICE vec3<T> tri_edge_coords(
...
template <typename T>
CPH_DEVICE void bary_is_edge(const vec3<T>& bary, const T eps, bool* is_edge, int* edge_idx_out) {
    if (bary[0] < eps) {
        *edge_idx_out = 1;
        *is_edge = true;
        return;
...
template <typename T>
CPH_DEVICE void bary_is_vert(const vec3<T>& bary, const T eps, bool* is_vert, int* vert_idx_out) {
    if (bary[0] > T(1) - eps) {
        *vert_idx_out = 0;
        *is_vert = true;
        return;
...
template <typename T>
CPH_DEVICE void trace_around_hole(
...
template <typename T>
CPH_DEVICE void trace_in_triangles(
    const Mesh<T>& mesh, const vec3<T>& dir_3d,
    const vec3<T>& curr_bary, const vec3<T>& curr_pos,
...
template <typename T>
CPH_DEVICE T signed_angle(const vec3<T>& A, const vec3<T>& B, const vec3<T>& N) {
...
template <typename T>
CPH_DEVICE vec3<T> get_next_bary(const Mesh<T>& mesh, int curr_tri, int next_tri, const vec3<T>& curr_bary, int* common) {
...
template <typename T>
CPH_DEVICE void compute_parallel_transport_edge(
    const Mesh<T>& mesh,
    int curr_tri,
    int next_tri,
    const vec3<T>& dir_3d,
...
template <typename T>
CPH_DEVICE void compute_parallel_transport_vertex_hole(
    const Mesh<T>& mesh,
    int curr_tri,
    int vertex_id,
    const vec3<T>& dir_3d,
...
template <typename T>
CPH_DEVICE void compute_parallel_transport_vertex(
    const Mesh<T>& mesh,
    int curr_tri,
    int vertex_id,
    const vec3<T>& dir_3d,
...
template <typename T>
CPH_DEVICE void project_vertex_dir(
...
template <typename T>
__global__ void straightest_geodesic_single(
    const Mesh<T> mesh,
    const torch::PackedTensorAccessor32<int,1,torch::RestrictPtrTraits> start_face,
    const torch::PackedTensorAccessor32<T,2,torch::RestrictPtrTraits> start_uvs,
    const torch::PackedTensorAccessor32<T,2,torch::RestrictPtrTraits> start_dirs,
    ...
    T eps,
    bool avoid_holes,
    torch::PackedTensorAccessor32<T,3,torch::RestrictPtrTraits> final_results,
    torch::PackedTensorAccessor32<T,4,torch::RestrictPtrTraits> debug_tensor
) {
    ...
    while (len_path < max_len_path - eps) {
        ...
        # if CTDA_DEBUG
            printf("Current triangle: %d, next position: [%.9g, %.9g, %.9g], next bary: [%.9g, %.9g, %.9g], dir: [%.9g, %.9g, %.9g]\n",
                curr_tri,
...
std::tuple<torch::Tensor, torch::Tensor> straightest_geodesics_cuda_impl(
    ...
    geodesic_utils::Mesh<scalar_t> mesh(
        mesh_positions.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>(),
        mesh_triangles.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
        mesh_adjacencies.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
        mesh_triangle_normals.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>(),
        mesh_v2t.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
        mesh_vertex_normals.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>()
    );
    ...
    straightest_geodesic_single<scalar_t><<<blocks, threads>>>(
        mesh,
        start_face.packed_accessor32<int, 1, torch::RestrictPtrTraits>(),
        start_uv.packed_accessor32<scalar_t, 2, torch::RestrictPtrTraits>(),
        start_dir.packed_accessor32<scalar_t, 2, torch::RestrictPtrTraits>(),
        num_starts,
        max_steps,
        debug,
        print_warnings,
...
std::tuple<torch::Tensor, torch::Tensor> straightest_geodesics_cuda(
    const torch::Tensor& mesh_positions,
    const torch::Tensor& mesh_triangles,
    const torch::Tensor& mesh_adjacencies,
    const torch::Tensor& mesh_triangle_normals,
    ...
    if (mesh_positions.scalar_type() == torch::kFloat) {
        return straightest_geodesics_cuda_impl<float>(
        ...
        return straightest_geodesics_cuda_impl<double>(
            mesh_positions, mesh_triangles, mesh_adjacencies,
            mesh_triangle_normals, mesh_v2t, mesh_vertex_normals,
            start_uv, start_dir, start_face,
...
TORCH_LIBRARY_IMPL(geodesic_utils, CUDA, m) {
    m.impl("straightest_geodesics", &straightest_geodesics_cuda);
}
